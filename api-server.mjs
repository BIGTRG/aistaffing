...
// ── Admin extensions (Add-On Services + Agent Workforce, PostgreSQL-backed) ──
import { initAdminExtensions, registerAdminRoutes } from './admin-extensions.mjs';
registerAdminRoutes(app, pool, authMiddleware);

// Start
const PORT = process.env.API_PORT || 3070;
initAdminExtensions(pool)
  .then(() => console.log('Admin extensions ready (addons + workforce)'))
  .catch((e) => console.error('Admin extensions init failed:', e.message));
